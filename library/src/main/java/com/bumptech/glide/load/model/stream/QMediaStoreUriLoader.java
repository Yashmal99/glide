 ...
 *
 * @param <Data> The type of data this loader will load ({@link InputStream}, {@link
 *     ParcelFileDescriptor}).
 */
@RequiresApi(Build.VERSION_CODES.Q)
public final class QMediaStoreUriLoader<Data> implements ModelLoader<Uri, Data> {
  private static final String[] PROJECTION = new String[] {MediaStore.MediaColumns.DATA};

  private final Context context;
  private final ModelLoader<File, Data> fileDelegate;
  private final ModelLoader<Uri, Data> uriDelegate;
  ...
  QMediaStoreUriLoader(
      Context context,
      ModelLoader<File, Data> fileDelegate,
      ModelLoader<Uri, Data> uriDelegate,
  ...
  private static final class QMediaStoreUriFetcher<Data> implements DataFetcher<Data> {
    ...
    @Nullable private volatile DataFetcher<Data> delegate;
    ...
    public void loadData(@NonNull Priority priority, @NonNull DataCallback<? super Data> callback) {
      try {
        DataFetcher<Data> local = buildDelegateFetcher();
        if (local == null) {
    ...
    @Nullable
    private DataFetcher<Data> buildDelegateFetcher() throws FileNotFoundException {
      LoadData<Data> result = buildDelegateData();
      return result != null ? result.fetcher : null;
    }

    @Nullable
    private LoadData<Data> buildDelegateData() throws FileNotFoundException {
    ...
    @Override
    public void cleanup() {
      DataFetcher<Data> local = delegate;
      if (local != null) {
        local.cleanup();
      }
    }
  ...
  private abstract static class Factory<Data> implements ModelLoaderFactory<Uri, Data> {

    private final Context context;
    private final Class<Data> dataClass;

    Factory(Context context, Class<Data> dataClass) {
    ...
    @NonNull
    @Override
    public final ModelLoader<Uri, Data> build(@NonNull MultiModelLoaderFactory multiFactory) {
